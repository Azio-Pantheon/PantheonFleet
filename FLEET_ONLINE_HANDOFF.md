...
| v1 panels | Farm map + printer status + history/jobs + **parts search** + **analytics (jobs + parts)** + **spool list (read-only)**. Archive panel hidden (file downloads impossible cross-site — bytes live on site NASes, no inbound path). Gcode browser deferred to fleet_daemon Track 2. |
...
Neon (cloud_sites, cloud_fleet_status, cloud_print_history[+dedup view],
      cloud_remoteprinters, cloud_vendor/filament/spool)
```
...
- **`cloud_vendor` / `cloud_filament` / `cloud_spool`** `(site, id INT, data JSONB, synced_at)` PK `(site,id)`. `data` = `row_to_json` of the local row. **Since 2026-09-18** the spool inventory is one cross-site dataset with globally unique ids (per-site SERIAL ranges): a spool is mirrored under its *physical* site (`data->>'site'`), a filament/vendor under the site that *created* it, and each row appears exactly once. The adapter therefore joins spool→filament→vendor on `id` alone (`api/spool/_spool.ts`, `api/spool/filaments.ts`); the `site` filter on `/api/spool/spools` now means "spools physically at that site". Spool fields the UI reads: `id, qr_code, initial_weight, used_weight, remaining_weight, loaded_on_printer, location, lot_nr, archived, filament_id`; filament: `id, vendor_id, name, material, color_hex, ...`; vendor: `id, name`.
